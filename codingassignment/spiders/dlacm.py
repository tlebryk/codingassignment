...
#     datefmt="%Y-%m-%d %H:%M:%S",
#     filename=f"C:/Users/tlebr/Google Drive/Personal/codingassignment/logs/dlacms2spider_{DATETIMENOW}.log",
#     level=logging.INFO,
# )
...
class DlAcmSearchSpider(scrapy.Spider):
    ...
    def __init__(self, **kwargs):
        ...
        self.start_urls = [
            ...
            for i in range(8)  # change to 8 later
        ]
    ...
    #     start_urls = df.googlescholar[df.googlescholar.notna()].str.replace(
    #         "citations?.*?user", "citations?user"
    #     )
    #     for url in self.start_urls:
    #         yield scrapy.Request(url.format(0), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for p in ppl:
            ...
            for link in links:
                ...
                if not data.get(key):
                    data[key] = value
            # data["dlcam_email"] = extract_text(p.xpath(".//a[contains(@data-title, 'Author’s Email')]").get())
            # data["dlcam_website"] = extract_text(p.xpath(".//a[contains(@data-title, 'Author’s Website')]").get())
            logging.info(data)
            yield data


# will need to drop duplicates later

# [print(a) for a in start_urls.apply(usercut)]


class DlAcmInstitutionSpider(scrapy.Spider):
    # search all people from uw
...
class DlAcmProfileSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for name, group in self.df.groupby("name"):  # TODO: remove []:1]
            for url in group.dlacm_profile:
                ...
                if name == "Anup Rao":
                    for i in range(5):
                        yield scrapy.Request(url2.format(i), callback=self.parse, meta=meta, cb_kwargs=cb_kwargs)
                # counter += 1
        #     return super().start_requests()
        # self.start_urls = self.df.dlacm_profile
        # super().__init__(**kwargs)
    ...
    def parse(self, response, baseurl, pg):
        ...
        for article in articles:
            ...
            data["facultyid"] = response.meta["id"]
            ...
            data["abstract"] = extract_text(article.css("div.issue-item__abstract").get())
            data["journal_name"] = extract_text(article.css("span.epub-section__title").get())
        ...
        #             "pg": pg+1,
        #         }
        #         yield response.follow(baseurl.format(pg+1), callback=self.parse, meta=response.meta, cb_kwargs=cb_kwargs)
    ...
    def endcheck(self, response):
        ...
        # puburl = response.urljoin(response.xpath("//a[contains(@title, 'View all Publications')]@href"))

        # scrapy.Request(arturl, callback=self.artparse)
    ...
    def artparse(self, response):
        ...
        data["abstract"] = extract_text(
            response.css("div.abstractSection").get()
        )
